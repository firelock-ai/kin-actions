...
def _guarded_symlink_paths(generated: Sequence[str]) -> set[str]:
    """Paths where a symlink could redirect generated bytes.

    A symlink matters here when it stands at a generated path or at any
    directory leading to one, because either can send a write somewhere other
    than the file the allowlist names.  Ancestors must be enumerated as path
    components rather than matched as a string prefix: ``ls-tree -r`` flattens
    directories, so a symlinked ``.github`` appears as one entry whose path is
    exactly ``.github`` and no prefix test against the longer generated path
    would ever see it.
    """
...
def _assert_safe_tree(
    ...
    guarded = _guarded_symlink_paths(generated)
    for path, entry in entries.items():
        if entry.mode == "120000" and path in guarded:
            raise InvariantError(
                f"{label} has a symlink where a generated file or one of its "
                f"parent directories belongs: {path}"
            )
...
def _require_allowlisted_train_delta(
    ...
    *,
    ...
    _assert_safe_tree(base_tree, "merge-base tree", generated)
    _assert_safe_tree(train_tree, "train tree", generated)
    _assert_safe_tree(main_tree, "trusted-main tree", generated)
...
def validate_merge(
    ...
    *,
    ...
    _assert_safe_tree(merge_tree, "merge tree", generated)
